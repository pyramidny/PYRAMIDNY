...
export default function TeamManagement() {
  ...
  async function handleSaveAssignment(projectId, pmId, apmId) {
    ...
    try {
      ...
      await proxyUpdateProject(
        projectId,
        { pm_id: pmId, assistant_pm_id: apmId },
        token
      );
      setProjects((prev) =>
        prev.map((p) =>
          p.id === projectId
            ? { ...p, pm_id: pmId, assistant_pm_id: apmId }
            : p
        )
      );
        ...
        <div>
          ...
          <div className="mt-6 p-4 bg-gray-800 rounded-xl border border-dashed border-gray-600">
            <p className="text-sm text-gray-400">
              <span className="text-yellow-400 font-semibold">16 staff pending</span> —
              awaiting Jorge's approval on role doc before bulk-loading into{" "}
