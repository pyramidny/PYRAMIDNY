...
export default function ProjectDetail() {
  ...
  const { session }  = useAuth();               // ← useAuth, NOT useMsal
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        // Direct reads — works if anon role has SELECT, otherwise swap to proxy action:"select"
        ...
        // Preload SharePoint folder contents
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {project.sharepoint_folder_url && (
              <a href={project.sharepoint_folder_url}
        ...
        <div className="flex gap-5 mt-3 text-sm text-gray-500">
          ...
          {project.division && (
            <span className="capitalize">{project.division}</span>
          )}
        </div>
      </div>
          ...
          {tabs.map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              ...
              {tab === "documents" && sp.files.length > 0 && (
                <span className="ml-1.5 bg-gray-100 text-gray-600 text-xs
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {[
            ...
            ["Contract Value",    project.contract_value
              ? `$${Number(project.contract_value).toLocaleString()}` : null],
            ["Start Date",        project.start_date
              ? new Date(project.start_date).toLocaleDateString() : null],
            ["Target Completion", project.target_completion
              ? new Date(project.target_completion).toLocaleDateString() : null],
          ].map(([label, value]) => (
      ...
      {activeTab === "tasks" && (
        <div className="space-y-2">
          ...
          {tasks.map((task) => (
            <label key={task.id}
              className="flex items-start gap-3 bg-white rounded-lg border border-gray-200
                         p-3.5 cursor-pointer hover:bg-gray-50 transition-colors">
              <input type="checkbox"
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-medium
                  ${task.status === "complete" ? "line-through text-gray-400" : "text-gray-900"}`}>
          ...
          {!project.sharepoint_folder_url ? (
            <div className="text-center py-16 text-gray-400">
              ...
              {!sp.configured && (
                <p className="text-xs mt-2 text-amber-600">
                  VITE_SP_SITE_ID / VITE_SP_DRIVE_ID not configured.
                </p>
              )}
            </div>
          ) : (
            <>
              <div className="flex items-center justify-between mb-4">
                ...
                <label className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm
              ...
              {sp.loading ? (
                ...
                <p className="text-sm text-gray-500 text-center py-8">
                  Folder is empty. Upload a file to get started.
                </p>
              ) : (
                <div className="space-y-1.5">
                  {sp.files.map((file) => (
                    <a key={file.id} href={file.webUrl}
                      target="_blank" rel="noopener noreferrer"
